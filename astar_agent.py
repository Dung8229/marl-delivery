...
class AStarAgents:
    ...
    def update_inner_state(self, state):
        ...
        for pkg_data in state['packages']:
            ...
            if pkg_id not in current_pkgs:
                # print(f"New package {pkg_id} at ({sr}, {sc}) to ({tr}, {tc})")
        ...
        for t in range(max_len):
            ...
            for a1 in paths:
                for a2 in paths:
                    ...
                    # a2 is idle
                    if len(p2) == 1:
                        # print(f"Idle agent {a2} at position {p2[0]} is idle, status: {self.robots_target[a2]}")
                        # If status is free means the agent has not been assigned to any package
                        # if self.robots_target[a2] != 'free':
                            # print(f"Idle agent {a2} at position {p2[0]} is not free, status: {self.robots_target[a2]}")
                        if self.robots_target[a2] == 'free':
                            # Check if the moving agent is going to the idle agent's position
                            if t < len(p1) and p1[t] == p2[0]:
                                # moving_pos = p1[t]
                                # print(f"Moving agent {a1} moving into idle agent {a2} at position {moving_pos}")
                                # idle_pos = p2[0]
                                # print(f"Idle agent {a2} at position {idle_pos}")
                                print(f"Idle conflict detected: {a1} moving into idle agent {a2} at time {t} at position {p2[0]}")
    ...
    def cbs(self, starts, goals):
        MAX_NODES = 200  # Giới hạn số lượng node để tránh lặp vô tận
        root = {'paths': {}, 'constraints': []}
        ...
        while open_list:
            ...
            if conflict_type == "idle_conflict":
                ...
                for agent in [a1, a2]:
                    ...
                    new_paths = dict(node['paths'])
                    new_path = self.a_star(starts[agent], goals[agent], self.build_constraints(new_constraints, agent))
                    if not new_path or new_path == node['paths'][agent]:
                        continue  # Bỏ qua nếu không tìm được path
                    new_paths[agent] = new_path
                    open_list.append({'paths': new_paths, 'constraints': new_constraints})

        return {}  # Trả về rỗng nếu không tìm được giải pháp
        ...
        for i in range(self.n_robots):
            ...
            # If the robot is not carrying a package, find the closest package
                ...
                if closest_package_id is not None:
                    print(f"Robot {i} found package {closest_package_id + 1} at ({self.packages[closest_package_id][1]}, {self.packages[closest_package_id][2]})")
                    ...
                    for j in range(len(self.packages)):
                        print(f"Package {j + 1} is {'free' if self.packages_free[j] else 'assigned'}")
                    # Set the goal to the pickup location
                    pkg = self.packages[closest_package_id]
                    goals.append((pkg[1], pkg[2]))
                    print(f"Robot {i} assigned to package {self.robots_target[i]} at ({pkg[1]}, {pkg[2]})")
